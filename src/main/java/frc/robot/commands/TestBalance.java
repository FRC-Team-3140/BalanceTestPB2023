...
public class TestBalance extends CommandBase {
    ...
    PIDController pid = new PIDController(0.45 / 15, 0.0, 0.002);
    ...
    public void execute() {
        ...
        // limit max speed
        double max_speed = settings.getDouble("balance_speed");

        speed = Math.min(Math.max(speed, -max_speed), max_speed);
        System.out.printf("speed=%.3f max_speed=%.3f\n", speed, max_speed);
    ...
    private void updateNetworkTableEntries() {
        ...
        if (balance_table.getEntry("max_speed").getDouble(max_speed) != max_speed) {
            max_speed = balance_table.getEntry("max_speed").getDouble(max_speed);
        }

        if (balance_table.getEntry("max_angle").getDouble(max_angle) != max_angle) {
            max_angle = balance_table.getEntry("max_angle").getDouble(max_angle);
        }

        if (balance_table.getEntry("pid_p").getDouble(pid_p) != pid_p) {
            pid_p = balance_table.getEntry("pid_p").getDouble(pid_p);
            pid.setP(pid_p);
        }

        if (balance_table.getEntry("pid_i").getDouble(pid_i) != pid_i) {
            pid_i = balance_table.getEntry("pid_i").getDouble(pid_i);
            pid.setI(pid_i);
        }

        if (balance_table.getEntry("pid_d").getDouble(pid_d) != pid_d) {
            pid_d = balance_table.getEntry("pid_d").getDouble(pid_d);
            pid.setD(pid_d);
        }

        // pid.setIntegratorRange(new_tc, new_pc);
    }
